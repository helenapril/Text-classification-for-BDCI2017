...
tf.flags.DEFINE_boolean('embedding_mode', False, 'whether to share across time-step')
...
def main(_):
    ...
    with tf.variable_scope("HANClassifierMode_%s_%s" % (FLAGS.mode, FLAGS.cell)):
        train_attention = HANClassifierModel(
            batch_queue_data[0],
            batch_queue_data[1],
            num_seq=32,
            num_classes=2,
        ...
        gradient, tvar = zip(*opt.compute_gradients(train_attention.loss))
        ...
        if FLAGS.embedding_mode:
            pre_trained_embedding = load_word()
            sess.run(train_attention.embedding_init, 
                     feed_dict={train_attention.embedding_placeholder: pre_trained_embedding})
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            for step in range(num_batches_train):
                batch_loss, ab, ac, ad, _, g_step = sess.run([
                    train_attention.loss,
                    train_attention.above,
                    train_attention.below,
                    train_attention.below_recall,
                    apply_gradient_op,
                    global_step])
