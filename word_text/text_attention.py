...
from highway_LSTM import HighwayLSTMCell, MultiplicativeLSTMCell, MulIntLSTMCell, VariationalDropoutWrapper
...
class HANClassifierModel():
    ...
    def __init__(self, inputs, labels, vocab_size, embedding_size, num_classes, num_seq, batch_size,
                 ...
                 dropout_keep_proba, filter_sizes, num_filters, choice, up, dropout_mode):
        ...
        with tf.variable_scope('hierarchical_attention'):
            ...
            if choice == 'rnn':
                with tf.variable_scope('forward', initializer=tf.orthogonal_initializer()):
                    cells_fw = dropout()
                    if dropout_mode:
                        cells_fw = VariationalDropoutWrapper(cells_fw, self.batch_size * self.num_seq,
                                                             keep_prob=self.dropout_keep_proba)
                with tf.variable_scope('backward', initializer=tf.orthogonal_initializer()):
                    cells_bw = dropout()
                    if dropout_mode:
                        cells_bw = VariationalDropoutWrapper(cells_bw, self.batch_size * self.num_seq,
                                                             keep_prob=self.dropout_keep_proba)
                word_level_inputs = tf.reshape(self.inputs_embedded,
                                               [self.batch_size * self.num_seq, self.sentence_size,
                                                self.embedding_size])
                self.word_lengths = tf.constant(16, shape=[batch_size * num_seq])
                word_encoder_output = bidirectional_rnn(cells_fw, cells_bw,
            ...
            with tf.variable_scope('sent_attention') as scope:
                with tf.variable_scope('forward', initializer=tf.orthogonal_initializer()):
                    cells_fw = dropout()
                    if dropout_mode:
                        cells_fw = VariationalDropoutWrapper(cells_fw, self.batch_size,
                                                             keep_prob=self.dropout_keep_proba)
                with tf.variable_scope('backward', initializer=tf.orthogonal_initializer()):
                    cells_bw = dropout()
                    if dropout_mode:
                        cells_bw = VariationalDropoutWrapper(cells_bw, self.batch_size,
                                                             keep_prob=self.dropout_keep_proba)
                if choice == 'cnn':
                ...
                sentence_encoder_output = bidirectional_rnn(
                    cells_fw, cells_bw, sentence_inputs,
                    tf.constant(self.num_seq, shape=[self.batch_size]))

                sentence_level_output = task_specific_attention(
                    sentence_encoder_output, 100)
                ...
                '''self.soft_logits = tf.nn.softmax(self.logits)
                list_logits = tf.split(self.soft_logits, num_or_size_splits=num_classes, axis=1)
                logit = tf.squeeze(list_logits[-1])
                ans = tf.greater_equal(logit, tf.constant(up))
                self.prediction = tf.cast(ans, dtype=tf.int64)'''
